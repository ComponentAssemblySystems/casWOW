...
- **Get Access**: If you're not already part of the Product and Design group, request access from the project lead or manager.
...
- **Get Access**: If you're not already configured with access to the Git repository, contact your manager for details.

### Development Tools

Your development environment is critical. Here are some tools and resources to help you set up your workspace:

- **Code Editor**: We recommend using [Visual Studio Code](https://code.visualstudio.com/) for contributing to the casWOW repository. It's highly customizable and supports a wide range of extensions.
...
- **Build Tools**: Familiarize yourself with our build tools and scripts, which are documented in the project's README file.

- **Frameworks**: This project uses [Eleventy](https://www.11ty.dev/) as the static site generator. For the frontend, we utilize a custom [Bootstrap](https://getbootstrap.com) library with [ChartJS](https://chartjs.org) to visualize data. [Storybook](https://storybook.js.org/) us used for local UI component development and testing.

- **Local Development**: Follow the setup instructions in the project's README file to get your local development environment up and running.
</div>
